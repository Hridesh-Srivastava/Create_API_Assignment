...
//route to render the main page(default)
app.get("/" , async(_ , res) => {
    try {
        ...
        res.status(500).json({
            message : "Error while rendering main page!"
        })
    }
});
...
app.get("/edit/:id" , async(req , res) => {
    try {
        const response = axios.get(`${API_URL}/posts/${req.params.id}`);
        res.render("modify.ejs" , {
            heading : "Edit Post",
            submit : "Update Post",
            post : response.data
...
app.post("/api/posts/:id" , async(req , res) => {
    try {
        ...
        res.status(500).json({
            message : "Error while updating this post!"
        });
    }
});
